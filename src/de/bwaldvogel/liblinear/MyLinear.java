 ...
 * @version 2014.9.28
 */
public class MyLinear extends Linear {
    ...
    public static double predictScore(Model model, Feature[] x) {
        int n;
        if (model.bias >= 0) {
        ...
        for (Feature lx : x) {
            ...
            if (idx <= n) {
                dec_values += w[(idx - 1)] * lx.getValue();
            }
        }
    ...
    public static double predictScore(Model model, double[] x) {
        int n;
        if (model.bias >= 0) {
            n = model.nr_feature + 1;
        } else {
    ...
    public static double predict(Model model, double[] xArray) {
        ArrayList<Feature> xtemp = new ArrayList<>();
        for (int i = 0; i < xArray.length; i++) {
            if (xArray[i] != 0) {
                xtemp.add(new FeatureNode(i + 1, xArray[i]));
            }
        }
        Feature[] x = new Feature[xtemp.size()];
    ...
    public static double predictProbability(Model model, Feature[] x, double[] prob_estimates) throws IllegalArgumentException {
        ...
        double label = predictValues2(model, x, prob_estimates);
        ...
        return prob_estimates[0];//label;
    }

    public static double predictValues2(Model model, Feature[] x, double[] dec_values) {
        int n;
        if (model.bias >= 0) {
            n = model.nr_feature + 1;
        } else {
        ...
        for (Feature[] nodes : prob.x) {
            ...
            for (Feature n : nodes) {
                if (n.getIndex() <= indexBefore) {
                    //System.out.println(n.getIndex()+" "+indexBefore);
