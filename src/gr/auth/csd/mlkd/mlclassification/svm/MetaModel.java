...
public class MetaModel {
    ...
    public MetaModel(int aNumber, String filenameTrainShell, String filenameTestShell, 
            int nr_features) {
    ...
    private ProblemGr loadProblem(String filenameShell, int nr_features) //load the problem (the dataset)
    {
        ...
        try {
            ...
            System.out.println("Null Problem! Exiting..");
            return null;
        }
    }
    ...
    public static double[] getMetaModelPrediction(String metalabelerFile, int numFeatures, 
            int M, String libsvmTestFile, Dictionary dictionary, Corpus test) {
        //vectorize testFile
        if (!new File(libsvmTestFile).exists()) {
            VectorizeJSON vectorize = new VectorizeJSON(dictionary, false, null);
            vectorize.vectorizeUnlabeled(test, libsvmTestFile);
        }

        //read metalabeler
        ProblemGr testProblem = ProblemGr.readProblem(libsvmTestFile, numFeatures);
...
//            if (metalabelerPredictions[m] < 10) metalabelerPredictions[m] = 10;
    ...
    public static double[] getMetaModelPrediction(Model metalabeler, int numFeatures, 
            int M, String libsvmTestFile, Dictionary dictionary, Corpus test) {
        //vectorize testFile
        //if (!new File(libsvmTestFile).exists()) {
            VectorizeJSON vectorize = new VectorizeJSON(dictionary, false, null);
