...
import de.bwaldvogel.liblinear.MyLinear;
...
public class SVM extends BinaryClassifier {
    ...
    public SVM(String trainLibSvm, String testLibSvm, int threads, int mod, int start, 
        ...
        //param = new Parameter(SolverType.L2R_L2LOSS_SVR_DUAL, C, E);
    ...
    public void learn(int label) {
        ...
        if (f.exists()||!labelValues.containsKey(label)) {
            System.out.println(label+" does not exist in labelvalues.");
            return;
        }
        //if(label ==1) System.out.println("Train: "+Arrays.deepToString(train.x));
        changeShell(label);
        if (tuned) {
            int pos = 0;
        ...
        model = MyLinear.train(train, param);
    ...
    public void predict(int label) {
        ...
        for (int i = 0; i < test.l; i++) {
            try {
                if (score) {
                    d = de.bwaldvogel.liblinear.MyLinear.predictScore(m, test.x[i]);
                    //double[] probs = new double[m.getNrClass()];
                    //d = de.bwaldvogel.liblinear.MyLinear.predictProbability(m, test.x[i], probs);
                } else {
